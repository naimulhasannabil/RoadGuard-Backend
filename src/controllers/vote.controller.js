...
import { VERIFICATION_THRESHOLD, CONTRIBUTION_POINTS, SOCKET_EVENTS } from "../config/constants.js"
import { updateUserLevel } from "../services/user.service.js"

const checkAndUpdateVerification = async (alertId, io) => {
  const alert = await prisma.alert.findUnique({
    where: { id: alertId },
    select: { id: true, upvotes: true, downvotes: true, isVerified: true, reporterId: true },
  })
  ...
  if (alert.upvotes >= VERIFICATION_THRESHOLD.MIN_UPVOTES && upvoteRatio >= VERIFICATION_THRESHOLD.MIN_VOTE_RATIO) {
    ...
    await prisma.user.update({
      ...
      data: { verifiedReports: { increment: 1 }, contributionScore: { increment: CONTRIBUTION_POINTS.ALERT_VERIFIED } },
    })
...
export const voteAlert = async (req, res, next) => {
  try {
    ...
    if (alert.reporterId === req.user.id) return res.status(400).json({ error: "Cannot vote on your own alert" })
    ...
    if (existingVote) {
      if (existingVote.isUpvote !== isUpvote) {
        await prisma.$transaction([
          prisma.alertVote.update({ where: { id: existingVote.id }, data: { isUpvote } }),
          prisma.alert.update({
            where: { id: alertId },
            data: {
      ...
      await prisma.$transaction([
        prisma.alertVote.create({ data: { userId: req.user.id, alertId, isUpvote } }),
        prisma.alert.update({
          where: { id: alertId },
          data: {
        ...
        prisma.user.update({
          ...
          data: { contributionScore: { increment: CONTRIBUTION_POINTS.VOTE_CAST } },
        }),
      ])
    }
    ...
    const updatedAlert = await prisma.alert.findUnique({
      where: { id: alertId },
      select: { id: true, upvotes: true, downvotes: true, isVerified: true, status: true },
    })

    io.emit(SOCKET_EVENTS.ALERT_UPDATED, updatedAlert)
    res.json({ message: isUpvote ? "Upvoted successfully" : "Downvoted successfully", alert: updatedAlert })
  } catch (error) {
    next(error)
  }
}
...
export const getVoteStatus = async (req, res, next) => {
  try {
    ...
    res.json({ hasVoted: !!vote, voteType: vote ? (vote.isUpvote ? "upvote" : "downvote") : null })
  } catch (error) {
    next(error)
  }
}
